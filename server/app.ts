...
export function buildApp({ config, db }: { config: Config; db: DbPool }) {
  ...
  app.post('/api/verification/send', { config: { rateLimit: { max: 4, timeWindow: '15 minutes' } } }, async request => {
    ...
    if (resend) {
      const logoUrl = `${config.APP_URL}/teachersvip-logo.png`
      const { error } = await resend.emails.send({ from: config.RESEND_FROM_EMAIL, to: workEmail, subject: 'Verify your TeachersVIP educator email', html: verificationEmailHtml(verificationUrl, logoUrl), text: `Verify your educator email\n\nConfirm your email to continue to your personalized TeachersVIP card and educator-only offers. This secure link expires in 30 minutes:\n\n${verificationUrl}\n\nIf you did not request this email, you can safely ignore it.\n\nFree for educators. Always.` })
      if (error) {
